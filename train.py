...
def step_regularize(fake_img, latents, mean_reg_variation, sqrt_one_minus_alpha_prod, decay=0.01):
    ...
    noise = torch.randn_like(fake_img) / math.sqrt(
        fake_img.shape[1] * fake_img.shape[2] * fake_img.shape[3]
    )
    noise = noise * sqrt_one_minus_alpha_prod
    grad, = autograd.grad(
        outputs=(fake_img * noise).sum(), inputs=latents, create_graph=True
    )
    reg_variations = torch.sqrt(grad.pow(2).sum(3).sum(2).sum(1))
...
def main():
    ...
    import shutil
    shutil.copyfile('./train.sh', os.path.join(args.output_dir, 'train.sh'))
    ...
    class ImageTextDataset(torch.utils.data.Dataset):
        ...
        def __init__(self, dataset_folder, regularization_file, tokenizer, transforms, args):
    ...
    with accelerator.main_process_first():
        train_dataset = ImageTextDataset(args.dataset_folder, args.regularization_annotation, tokenizer, train_transforms, args)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # accelerator.print('prediction type', noise_scheduler.config.prediction_type)
            ...
            if accelerator.sync_gradients:
                progress_bar.update(1)
                global_step += 1
                accelerator.log({"train_loss": train_loss, "train_reg_loss": train_reg_loss}, step=global_step)
                train_loss = 0.0
                train_reg_loss = 0.0
            ...
            logs = {"step_loss": loss.detach().item(), "reg_loss": reg_loss.detach().item(), "reg_variation": reg_variations.mean().detach().item(), "mean_reg_variation": mean_reg_variation.item(), "lr": lr_scheduler.get_last_lr()[0]}
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"Mean reg variation = {mean_reg_variation.item()}")
